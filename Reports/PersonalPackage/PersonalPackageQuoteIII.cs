...
{
    /// <summary>
    /// Summary description for PersonalPackageQuoteIII.
    /// </summary>
    public partial class PersonalPackageQuoteIII : DataDynamics.ActiveReports.ActiveReport3
    {
        private int _iRow = 0;
        ...
        public PersonalPackageQuoteIII(System.Collections.ArrayList AutoCover, EPolicy.TaskControl.QuoteAuto taskControl)
        {
            _policy = taskControl;
            this._iRow = 0;
        ...
        private void PersonalPackageQuoteIII_FetchData(object sender, FetchEventArgs eArgs)
        {
            //txtCotizacionNo.Text = _policy.TaskControlID.ToString().Trim();
            //TxtFecha.Text = _policy.EntryDate.ToShortDateString();
            ...
            this.txtWork.Text = 
                ...
                (int)Tables.CITY);*/
            this.txtClass.Text = autoCover.VehicleClass;
            ...
            this.txtTotalMedicalPayment.Text = String.Format("{0:c}",
                autoCover.MedicalPremium());
            ...
            this.txtAllPrima.Text = String.Format("{0:c}",
                   this.GetPremium(_policy));

            decimal charges = this.GetCharges(_policy);
            this.txtCharge.Text = String.Format("{0:c}",
                 charges);

            decimal totalPremium = charges + _policy.TotalPremium;
            this.txtTotPrem.Text = String.Format("{0:c}",
                 totalPremium);

            SetPointCode(autoCover);

            this._iRow++;
            eArgs.EOF = false;
        }

        private void SetPointCode(AutoCover autoCover)
        {
            if(autoCover.ComprehensiveDeductible != 0)
                txtPCComp.Text = PrintPolicy.GetPointCode("Auto", "ComprehensiveDeductible", autoCover.ComprehensiveDeductible.ToString().Trim());

            if (autoCover.CollisionDeductible != 0)
                txtPCColl.Text = PrintPolicy.GetPointCode("Auto", "CollissionDeductible", autoCover.CollisionDeductible.ToString().Trim());

            if (autoCover.BodilyInjuryLimit != 0)
                txtPCBodilyInjury.Text = PrintPolicy.GetPointCode("Auto", "BodilyInjuryLimit", autoCover.BodilyInjuryLimit.ToString().Trim());

            if (autoCover.PropertyDamageLimit != 0)
                txtPCPropertyDamage.Text = PrintPolicy.GetPointCode("Auto", "PropertyDamageLimit", autoCover.PropertyDamageLimit.ToString().Trim());

            if (autoCover.MedicalLimit != 0)
                txtPCMedicalPay.Text = PrintPolicy.GetPointCode("Auto", "MedicalLimit", autoCover.MedicalLimit.ToString().Trim());
        }
        ...
        private enum Tables
        {
            ...
            PROPERTY_DAMAGE_LIMIT, COMBINED_SINGLE_LIMIT
        };
